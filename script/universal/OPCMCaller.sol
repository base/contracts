...
///     │        │        │        │        │        │          │            run()             │
///     │        │        │        │        │        │          │─────────────────────────────>│
abstract contract OPCMCaller is Script {
    ...
    /// @notice Returns the address of the OPCM contract to call
    function _opcmAddress() internal view virtual returns (address);

    /// @notice Creates the calldata for signatures (`sign`), approvals (`approve`), and execution (`run`)
    function _buildCall() internal view virtual returns (IMulticall3.Call3 memory);
    ...
    function sign(address[] memory safes) public {
        ...
        bytes[] memory datas = _transactionDatas({safes: safes});

        vm.startMappingRecording();
        (Vm.AccountAccess[] memory accesses, Simulation.Payload memory simPayload) =
            _simulateForSigner({safes: safes, datas: datas});
        ...
        bytes memory txData = _encodeTransactionData({safe: safes[0], data: datas[0]});
        StateDiff.recordStateDiff({json: json, parents: parents, txData: txData, targetSafe: _ownerSafe()});

        _printDataToSign({safe: safes[0], data: datas[0], txData: txData});
    }
    ...
    function verify(address[] memory safes, bytes memory signatures) public view {
        safes = _appendOwnerSafe({safes: safes});
        (bytes[] memory datas) = _transactionDatas({safes: safes});
        _checkSignatures({safe: safes[0], data: datas[0], signatures: signatures});
    }
    ...
    function approve(address[] memory safes, bytes memory signatures) public {
        ...
        (Vm.AccountAccess[] memory accesses, Simulation.Payload memory simPayload) = _executeTransaction({
            safe: safes[0], to: MULTICALL3_ADDRESS, data: datas[0], signatures: signatures, broadcast: true
        });
    ...
    function simulate(bytes memory signatures) public {
        ...
        bytes[] memory datas = _transactionDatas({safes: _toArray(ownerSafe)});

        vm.store({target: ownerSafe, slot: SAFE_NONCE_SLOT, value: bytes32(_getNonce({safe: ownerSafe}))});

        (Vm.AccountAccess[] memory accesses, Simulation.Payload memory simPayload) = _executeTransaction({
            safe: ownerSafe, to: _opcmAddress(), data: datas[0], signatures: signatures, broadcast: false
        });
    ...
    function run(bytes memory signatures) public {
        ...
        (Vm.AccountAccess[] memory accesses, Simulation.Payload memory simPayload) = _executeTransaction({
            safe: ownerSafe, to: _opcmAddress(), data: datas[0], signatures: signatures, broadcast: true
        });
    ...
    function _transactionDatas(address[] memory safes) private view returns (bytes[] memory datas) {
        // Build the calls and sum the values
        IMulticall3.Call3 memory call = _buildCall();

        // The very last call is the actual (aggregated) call to execute
        datas = new bytes[](safes.length);
        datas[datas.length - 1] = call.callData;
        ...
        for (uint256 i = safes.length - 1; i > 0; i--) {
            ...
            approvalCall[0] = _generateApproveCall({safe: targetSafe, data: callToApprove});
            datas[i - 1] = abi.encodeCall(IMulticall3.aggregate3, (approvalCall));
        }
    }

    function _generateApproveCall(address safe, bytes memory data) internal view returns (IMulticall3.Call3 memory) {
        bytes32 hash = _getTransactionHash({safe: safe, data: data});
    ...
    function _printDataToSign(address safe, bytes memory data, bytes memory txData) internal {
        bytes32 hash = _getTransactionHash({safe: safe, data: data});
    ...
    function _executeTransaction(address safe, address to, bytes memory data, bytes memory signatures, bool broadcast)
        internal
        returns (Vm.AccountAccess[] memory, Simulation.Payload memory)
    {
        ...
        bytes memory simData = _execTransactionCalldata({safe: safe, data: data, signatures: signatures});
        Simulation.logSimulationLink({to: safe, data: simData, from: msg.sender});

        vm.startStateDiffRecording();
        bool success = _execTransaction({safe: safe, to: to, data: data, signatures: signatures, broadcast: broadcast});
    ...
    function _simulateForSigner(address[] memory safes, bytes[] memory datas)
        internal
        returns (Vm.AccountAccess[] memory, Simulation.Payload memory)
    {
        IMulticall3.Call3[] memory calls = _simulateForSignerCalls({safes: safes, datas: datas});

        bytes32 firstCallDataHash = _getTransactionHash({safe: safes[0], data: datas[0]});
        ...
        Simulation.logSimulationLink({to: _opcmAddress(), data: txData, from: msg.sender, overrides: overrides});
        ...
        Simulation.Payload memory simPayload =
            Simulation.Payload({to: _opcmAddress(), data: txData, from: msg.sender, stateOverrides: overrides});
        Vm.AccountAccess[] memory accesses = Simulation.simulateFromSimPayload({simPayload: simPayload});
        return (accesses, simPayload);
    }

    function _simulateForSignerCalls(address[] memory safes, bytes[] memory datas)
        private
        view
        ...
        for (uint256 i; i < safes.length; i++) {
            ...
            calls[i] = IMulticall3.Call3({
                ...
                callData: _execTransactionCalldata({
                    safe: safes[i], data: datas[i], signatures: Signatures.genPrevalidatedSignature(signer)
                })
            });
        }
    ...
    function _checkSignatures(address safe, bytes memory data, bytes memory signatures) internal view {
        bytes32 hash = _getTransactionHash({safe: safe, data: data});
        signatures = Signatures.prepareSignatures({safe: safe, hash: hash, signatures: signatures});
        IGnosisSafe(safe).checkSignatures({dataHash: hash, data: data, signatures: signatures});
    }

    function _getTransactionHash(address safe, bytes memory data) internal view returns (bytes32) {
        return keccak256(_encodeTransactionData({safe: safe, data: data}));
    }

    function _encodeTransactionData(address safe, bytes memory data) internal view returns (bytes memory) {
    ...
    function _execTransactionCalldata(address safe, bytes memory data, bytes memory signatures)
        internal
        view
        returns (bytes memory)
    {
        return abi.encodeCall(
            ...
            (_opcmAddress(), 0, data, Enum.Operation.DelegateCall, 0, 0, 0, address(0), payable(address(0)), signatures)
        );
    }

    function _execTransaction(address safe, address to, bytes memory data, bytes memory signatures, bool broadcast)
        internal
        returns (bool)
    {
        if (broadcast) {
