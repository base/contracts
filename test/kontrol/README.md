...
└── <a href="./scripts">scripts</a>: Where the scripts of the projects live
    ├── <a href="./scripts/json">json</a>: Data cleaning scripts for the output of <a href="../../scripts/deploy/SystemDeploy.s.sol">SystemDeploy.s.sol</a>
    ├── <a href="./scripts/make-summary-deployment.sh">make-summary-deployment.sh</a>: Executes <a href="../../scripts/deploy/SystemDeploy.s.sol">SystemDeploy.s.sol</a>, curates the result and writes the summary deployment contract
...
First, generate a deployment summary contract from [`SystemDeploy.s.sol`](../../scripts/deploy/SystemDeploy.s.sol) by running the following command:
...
This step is optional if an up-to-date summary contract already exists, which will be the case until the `SystemDeploy` deployment flow changes, or any of the source contracts under test change.
...
The summary contract can be found in [`DeploymentSummary.sol`](./proofs/utils/DeploymentSummary.sol), which summarizes the state changes from `SystemDeploy.runWithStateDiff`.
...
These are the instructions to add a new proof to this project. If all functions involved in the new proof are from a contract already deployed by `SystemDeploy.runWithStateDiff`, the first two steps can be skipped.

#### Make Kontrol aware of the new contract being tested

The `runWithStateDiff` function of [`SystemDeploy.s.sol`](../../scripts/deploy/SystemDeploy.s.sol) reproduces the deployment process used by the Foundry test harness. It has the `stateDiff` modifier to make use of [Foundry's state diff cheatcodes](https://book.getfoundry.sh/cheatcodes/start-state-diff-recording). Kontrol utilizes the JSON resulting from this modifier for two purposes:
1. Load all the state updates generated by `runWithStateDiff` as the initial configuration for all proofs, effectively offloading the computation of the deployment process to `forge` and thus improving performance.
...
Once new deployment steps have been added to `SystemDeploy` the state-diff files have to [be rebuilt](#build-deployment-summary).
...
- `kdeploy`: Used by [`make-summary-deployment.sh`](./scripts/make-summary-deployment.sh) to generate the [`DeploymentSummary.sol`](./proofs/utils/DeploymentSummary.sol) contract based on execution of the [`SystemDeploy.s.sol`](../../scripts/deploy/SystemDeploy.s.sol) contract using Foundry's state diff recording cheatcodes.
...
The `make-summary-deployment.sh` script saves off the generated JSON state diff to `snapshots/state-diff/Kontrol-31337.json`, and is run as part of the `snapshots` script in `package.json`.
